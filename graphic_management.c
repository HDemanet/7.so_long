...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/08/09 14:58:04 by hdemanet          #+#    #+#             */
/*   Updated: 2024/09/10 12:51:54 by hdemanet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_image_to_window(t_game *game)
{
	...
	while (i < game->map->height)
	{
		...
		{
			put_image(game, game->map->vector[i][j], (j * 49), (i * 49));
			j++;
		}
		i++;
	}
...
void	create_game_interface(t_game *game)
{
	...
	game->mlx_window = mlx_new_window((game->mlx), (game->map->width * 49),
			(game->map->height * 49), "./so_long");
	if (!(game->mlx_window))
